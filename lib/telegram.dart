...
class _TelegramSpamPageState extends State<TelegramSpamPage>
  ...
  // --- Color Scheme ---
  static const Color _primaryGreen = Color(0xFF2196F3);
  ...
  Future<void> _loadSessions() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('https://ppl.nullxteam.fun/api/tools/telegram/sessions?key=${widget.sessionKey}'),
      );
  ...
  Future<void> _initiateLogin({bool isResend = false}) async {
    ...
    try {
      ...
      final response = await http.get(
        Uri.parse('https://ppl.nullxteam.fun/api/tools/telegram/login?key=${widget.sessionKey}&phone=$phone'),
      );
  ...
  Future<void> _submitAuth() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('https://ppl.nullxteam.fun/api/tools/telegram/auth?key=${widget.sessionKey}&loginId=$_currentLoginId&input=${_authController.text.trim()}'),
      );
  ...
  void _startLoginStatusPolling() {
    ...
    _loginStatusTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
      try {
        final response = await http.get(
          Uri.parse('https://ppl.nullxteam.fun/api/tools/telegram/status?key=${widget.sessionKey}&loginId=$_currentLoginId'),
        );
        final data = jsonDecode(response.body);
  ...
  Future<void> _verifySessionPassword(String phone) async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://ppl.nullxteam.fun/api/tools/telegram/verify-session-password'),
  ...
  Future<void> _refreshSessions() async {
    ...
    try {
      final response = await http.get(
        Uri.parse('https://ppl.nullxteam.fun/api/tools/telegram/refresh-sessions?key=${widget.sessionKey}'),
      );
      final data = jsonDecode(response.body);
      if (data['valid'] == true) {
  ...
  Future<void> _deleteSession(String phone) async {
    try {
      final response = await http.get(
        Uri.parse('https://ppl.nullxteam.fun/api/tools/telegram/remove-ses?key=${widget.sessionKey}&phone=$phone'),
      );
      final data = jsonDecode(response.body);
      if (data['valid'] == true) {
  ...
  Future<void> _startSpamReport() async {
    ...
    try {
      final response = await http.post(
        Uri.parse('https://ppl.nullxteam.fun/api/tools/telegram/spam-report'),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'key': widget.sessionKey,
  ...
  void _startStatusPolling() {
    ...
    _statusCheckTimer = Timer.periodic(const Duration(seconds: 2), (timer) async {
      try {
        final response = await http.get(
          Uri.parse('https://ppl.nullxteam.fun/api/tools/telegram/report-status?key=${widget.sessionKey}&reportId=$_currentReportId'),
        );
        final data = jsonDecode(response.body);
        if (data['valid'] == true) {
  ...
  Widget build(BuildContext context) {
    return Card(
      ...
      shape: RoundedRectangleBorder(
        ...
        side: BorderSide(
          color: widget.session.isActive ? const Color(0xFF2196F3).withOpacity(0.3) : Colors.red.withOpacity(0.3),
        ),
      ),
      child: Column(
        children: [
          ListTile(
            leading: Icon(
              Icons.phone_android,
              color: widget.session.isActive ? const Color(0xFF2196F3) : Colors.red,
            ),
            title: Text(
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            const Text('Report Configuration', style: TextStyle(color: Color(0xFF2196F3), fontSize: 16, fontWeight: FontWeight.bold)),
            const SizedBox(height: 16),
            TextField(
    ...
    return Card(
      ...
      child: Padding(
        ...
        child: Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            const Text('Report Progress', style: TextStyle(color: Color(0xFF2196F3), fontSize: 16, fontWeight: FontWeight.bold)),
